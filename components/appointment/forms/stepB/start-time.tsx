...
export type StartTimeFormValues = z.infer<typeof formSchema>;
...
export function StartTimeForm({
  ...
  const form = useForm<StartTimeFormValues>({
  ...
  const onSubmit = async (data: StartTimeFormValues) => {
    try {
      setLoading(true);

      await axios.patch(`/api/appointments/${appointment.id}/stepB`, data);
