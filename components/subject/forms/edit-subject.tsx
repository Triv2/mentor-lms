'use client'
import {  Course, Profile, Subject } from '@prisma/client';
...
export type EditSubjectFormValues = z.infer<typeof formSchema>
...
const form = useForm<EditSubjectFormValues>({
  ...
  defaultValues: {
    ...
    cost: parseFloat(String(subject?.cost)) || 0,
...
const onSubmit = async (data:EditSubjectFormValues) => {
  try {
    ...
    await axios.patch(`/api/subjects/${subject.id}`, data)
  ...
  return (
    ...
    <div >
          ...
          render={({ field }) => (
            <FormItem className="flex flex-col">
              ...
              <FormLabel className="text-xs text-muted-foreground flex justify-between px-2">
                Current Name: <p className="font-semibold text-emerald-800 dark:text-emerald-500">{subject?.name}</p>
              </FormLabel>
              <FormControl>
               <Input 
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="flex flex-col">
              ...
              <FormLabel className="text-xs text-muted-foreground flex justify-between px-2">
                Current Cost: <p className="font-semibold text-emerald-800 dark:text-emerald-500 pl-2"> {subject?.cost}</p>
              </FormLabel>
              <FormControl>
        ...
        <div className="w-full">
          ...
          render={({ field }) => (
            <FormItem className="flex flex-col">
              ...
              <FormLabel className="text-xs text-muted-foreground flex justify-between px-2">
                Current Description: <p className="font-semibold text-emerald-800 dark:text-emerald-500 pl-2 truncate w-[200px]"> {subject?.description}</p>
              </FormLabel>
              <FormControl>
        ...
        <div className="flex items-center flex-col justify-center">
          <Switch defaultSelected size="sm" onClick={()=>handleClick()}><p className="text-xs">Change Subject Image?</p></Switch>
          {upload && (
        <FormField
          control={form.control}
          name="imageUrl"
