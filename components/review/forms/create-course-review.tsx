...
interface CreateCourseReviewFormProps {
  mentor: Mentor;
  profile: Profile;
  course: Course;
  onClose: () => void;
...
const formSchema= z.object({
  ...
  courseId: z.string().min(1).max(1000),
});

export type CreateCourseReviewFormValues = z.infer<typeof formSchema>

const CreateCourseReviewForm = ({
  mentor,
  onClose,
  profile,
  course,
}:CreateCourseReviewFormProps) => {
...
const form = useForm<CreateCourseReviewFormValues>({
...
const onSubmit = async (data:CreateCourseReviewFormValues) => {
  try {
    setLoading(true);
    
  
    await axios.post(`/api/courses/${course.id}/reviews`, data)
