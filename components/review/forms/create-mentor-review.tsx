...
interface CreateMentorReviewFormProps {
  mentor: Mentor;
  profile: Profile;
  onClose: () => void;
}
...
export type CreateMentorReviewFormValues = z.infer<typeof formSchema>

const CreateMentorReviewForm = ({
  mentor,
  onClose,
  profile,
}:CreateMentorReviewFormProps) => {
...
const form = useForm<CreateMentorReviewFormValues>({
...
const onSubmit = async (data:CreateMentorReviewFormValues) => {
  try {
    setLoading(true);
    
  
    await axios.post(`/api/mentors/${mentor.id}/reviews`, data)
...
export default CreateMentorReviewForm;