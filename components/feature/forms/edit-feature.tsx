'use client'
import {  Course, Feature, Profile } from '@prisma/client';
...
export type EditFeatureFormValues = z.infer<typeof formSchema>
...
const onSubmit = async (data:EditFeatureFormValues) => {
  try {
    setLoading(true);
    
   
    await axios.patch(`/api/features/${feature.id}`,data)
          ...
          render={({ field }) => (
            <FormItem className="flex flex-col">
              ...
              <FormLabel className="text-xs text-muted-foreground flex justify-between px-2">
                Current Title: <p className="font-semibold text-emerald-800 dark:text-emerald-500">{feature?.title}</p>
              </FormLabel>
              <FormControl>
               <Input 
        ...
        <div className="w-full">
          ...
          render={({ field }) => (
            <FormItem className="flex flex-col">
              ...
              <FormLabel className="text-xs text-muted-foreground flex justify-between px-2">
                Current Description: <p className="font-semibold text-emerald-800 dark:text-emerald-500">{feature?.description}</p>
              </FormLabel>
              <FormControl>
