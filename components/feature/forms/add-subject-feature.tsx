...
interface AddSubjectFeatureFormProps {
...
export type AddSubjectFeatureFormValues = z.infer<typeof formSchema>

const AddSubjectFeatureForm = ({
  onClose,
  subject,
}:AddSubjectFeatureFormProps) => {
...
const form = useForm<AddSubjectFeatureFormValues>({
...
const onSubmit = async (data:AddSubjectFeatureFormValues) => {
  try {
    setLoading(true);
    
  
    await axios.post(`/api/subjects/${subject.id}/features`, data)
    
    
    toast.success("Feature added. Subject updated!");
  } catch (error) {
...
export default AddSubjectFeatureForm;
